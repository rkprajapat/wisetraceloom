...
`quickstart.py` — runnable, end-to-end tour of trailwise's public API:
...
from trailwise's own fail-open guarantee), and per-ticket multi-tenant
context via `bind_context`.

Run either from the repo root (as a module, so `trailwise/` resolves off the
...
once any script sets a `log_file_path`, every later `trailwise.configure()`
call across *any* script sharing that database (the default is
`.trailwise/trailwise.db` under the repo root) resolves to that file
...
persisted config — see `trailwise.set_db_path` in the README if you want an
