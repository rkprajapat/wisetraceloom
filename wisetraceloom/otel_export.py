...
Maps `wisetraceloom.schema`'s internal rich spans (`AgentSpan`, `ToolSpan`,
...
same store as `wisetraceloom.config`) rather than an environment variable, so
...
OTel's own generated trace/span ids and carry the original `wisetraceloom.*`
...
produces a standalone OTel span; the `wisetraceloom.parent_span_id` attribute
...
def _set_correlation_attrs(otel_span: trace.Span, span: AgentSpan | ToolSpan | LLMSpan) -> None:
    otel_span.set_attribute("wisetraceloom.schema_version", span.schema_version)
    otel_span.set_attribute("wisetraceloom.trace_id", span.trace_id)
    otel_span.set_attribute("wisetraceloom.span_id", span.span_id)
    if span.parent_span_id:
        otel_span.set_attribute("wisetraceloom.parent_span_id", span.parent_span_id)
    if span.tenant_id:
        otel_span.set_attribute("wisetraceloom.tenant_id", span.tenant_id)
...
def export_agent_span(span: AgentSpan, *, tracer_provider: TracerProvider | None = None) -> None:
    ...
    # Not part of gen_ai.* (semconv has no loop-guard concept) — wisetraceloom's
    # own runaway-loop signal (PRD §4, §7).
    otel_span.set_attribute("wisetraceloom.loop_iteration", span.loop_iteration)
...
def export_llm_span(
    ...
    *,
    ...
    # Cost/prompt-version attribution: wisetraceloom-specific, no gen_ai.* home yet.
    if span.estimated_cost_usd is not None:
        otel_span.set_attribute("wisetraceloom.estimated_cost_usd", span.estimated_cost_usd)
    if span.prompt_version_id:
        otel_span.set_attribute("wisetraceloom.prompt_version_id", span.prompt_version_id)
    _set_correlation_attrs(otel_span, span)
